...
final class TenantSecurityRequest implements Closeable {
  ...
  private TenantSecurityException parseFailureFromRequest(HttpResponse resp) {
    if (resp.getStatusCode() == HttpStatusCodes.STATUS_CODE_UNAUTHORIZED) {
      ...
      return new TspServiceException(
          TenantSecurityErrorCodes.UNAUTHORIZED_REQUEST, resp.getStatusCode());
    }
    try {
   ...
   */
  private <T> CompletableFuture<T> makeRequestAndParseFailure(
  ...
  CompletableFuture<WrappedDocumentKey> wrapKey(DocumentMetadata metadata) {
    ...
    String error =
        String.format(
            ...
            this.wrapEndpoint);
    return this.makeRequestAndParseFailure(
        this.wrapEndpoint, postData, WrappedDocumentKey.class, error);
  }

  /** Request batch wrap endpoint to generate the provided number of DEK/EDEK pairs. */
  CompletableFuture<BatchWrappedDocumentKeys> batchWrapKeys(
      Collection<String> documentIds, DocumentMetadata metadata) {
    Map<String, Object> postData = metadata.getAsPostData();
    postData.put("documentIds", documentIds);
    String error =
        String.format(
            "Unable to make request to Tenant Security Proxy batch wrap endpoint. Endpoint"
                + " requested: %s",
            this.batchWrapEndpoint);
    return this.makeRequestAndParseFailure(
        this.batchWrapEndpoint, postData, BatchWrappedDocumentKeys.class, error);
  }
  ...
  CompletableFuture<byte[]> unwrapKey(String edek, DocumentMetadata metadata) {
    ...
    String error =
        String.format(
            "Unable to make request to Tenant Security Proxy unwrap endpoint. Endpoint requested:"
                + " %s",
            this.unwrapEndpoint);
    return this.makeRequestAndParseFailure(
            this.unwrapEndpoint, postData, UnwrappedDocumentKey.class, error)
        .thenApply(
            unwrapResponse -> {
              try {
   ...
   */
  CompletableFuture<BatchUnwrappedDocumentKeys> batchUnwrapKeys(
      Map<String, String> edeks, DocumentMetadata metadata) {
    Map<String, Object> postData = metadata.getAsPostData();
    postData.put("edeks", edeks);
    String error =
        String.format(
            "Unable to make request to Tenant Security Proxy batch unwrap endpoint. Endpoint"
                + " requested: %s",
            this.batchWrapEndpoint);
    return this.makeRequestAndParseFailure(
        this.batchUnwrapEndpoint, postData, BatchUnwrappedDocumentKeys.class, error);
  }
   ...
   * to the newTenantId.
   */
  CompletableFuture<RekeyedDocumentKey> rekey(
    ...
    String error =
        String.format(
            "Unable to make request to Tenant Security Proxy rekey endpoint. Endpoint requested:"
                + " %s",
            this.rekeyEndpoint);
    return this.makeRequestAndParseFailure(
        this.rekeyEndpoint, postData, RekeyedDocumentKey.class, error);
  }
  ...
  CompletableFuture<Void> logSecurityEvent(SecurityEvent event, EventMetadata metadata) {
    ...
    String error =
        String.format(
            "Unable to make request to Tenant Security Proxy security event endpoint. Endpoint"
                + " requested: %s",
            this.securityEventEndpoint);
    return this.makeRequestAndParseFailure(this.securityEventEndpoint, postData, Void.class, error);
  }

  private Map<String, Object> combinePostableEventAndMetadata(
      SecurityEvent event, EventMetadata metadata) {
    Map<String, Object> postData = metadata.getAsPostData();
