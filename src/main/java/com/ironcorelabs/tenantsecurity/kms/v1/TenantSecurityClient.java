...
public final class TenantSecurityClient implements Closeable {
         ...
         * @throws Exception If the provided domain is invalid.
         */
        public TenantSecurityClient(String tspDomain, String apiKey, int requestThreadSize, int aesThreadSize)
                        throws Exception {
         ...
         */
        public TenantSecurityClient(String tspDomain, String apiKey, int requestThreadSize, int aesThreadSize,
                        SecureRandom randomGen, int timeout) throws Exception {
                ...
                this.encryptionService = new TenantSecurityRequest(tspDomain, apiKey, requestThreadSize, timeout);
        ...
        private static byte[] generateHeader() {
                ...
                return ByteBuffer.allocate(DOCUMENT_HEADER_META_LENGTH).put(headerVersion).put(magic).put(headerSize)
                                .array();
        }
        ...
        private CompletableFuture<byte[]> encryptBytes(byte[] document, byte[] documentKey) {
                ...
                return CompletableFutures.tryCatchNonFatal(() -> {
                        ...
                        return ByteBuffer.allocate(header.length + IV_BYTE_LENGTH + encryptedBytes.length).put(header)
                ...
                Map<String, CompletableFuture<byte[]>> encryptOps = document.entrySet().stream()
                                .collect(Collectors.toMap(Map.Entry::getKey, entry -> {
                                        ...
                                        return CompletableFuture.supplyAsync(
                                                        () -> encryptBytes(entry.getValue(), dek).join(),
                                                        encryptionExecutor);
                                }));
                                ...
                                .collect(Collectors.toMap(Map.Entry::getKey, entry -> {
                                        ...
                                        return CompletableFuture.supplyAsync(
                                                        () -> parseDocumentParts(entry.getValue())
                                                                        .thenCompose(encryptedDocument -> decryptBytes(
                                                                                        encryptedDocument, dek))
                                                                        .join(),
                                                        encryptionExecutor);
                                }));
        ...
        private ConcurrentMap<String, EncryptedDocument> encryptBatchOfDocuments(
                        Map<String, Map<String, byte[]>> documents, ConcurrentMap<String, WrappedDocumentKey> dekList) {
        ...
        public CompletableFuture<BatchResult<EncryptedDocument>> encryptBatch(
                ...
                return this.encryptionService.batchWrapKeys(plaintextDocuments.keySet(), metadata)
                                .thenComposeAsync(batchResponse -> {
                                        ...
                                        return CompletableFuture.supplyAsync(
                                                        () -> encryptBatchOfDocuments(plaintextDocuments, dekList))
                                                        .thenCombine(CompletableFuture.supplyAsync(
                                                                        () -> getBatchFailures(failureList)),
                                                                        (s, f) -> new BatchResult<EncryptedDocument>(s,
                                                                                        f));
                                }, encryptionExecutor);
        }
        ...
        public CompletableFuture<BatchResult<EncryptedDocument>> encryptExistingBatch(
                ...
                return this.encryptionService.batchUnwrapKeys(edekMap, metadata).thenComposeAsync(batchResponse -> {
                        ConcurrentMap<String, UnwrappedDocumentKey> dekList = new ConcurrentHashMap<>(
                                        batchResponse.getKeys());
                        ConcurrentMap<String, ErrorResponse> failureList = new ConcurrentHashMap<>(
                                        batchResponse.getFailures());
                        return CompletableFuture
                                        .supplyAsync(() -> encryptExistingBatchOfDocuments(plaintextDocuments, dekList))
        ...
        public CompletableFuture<BatchResult<PlaintextDocument>> decryptBatch(
                ...
                return this.encryptionService.batchUnwrapKeys(edekMap, metadata).thenComposeAsync(batchResponse -> {
                        ...
                        ConcurrentMap<String, ErrorResponse> failureList = new ConcurrentHashMap<>(
                                        batchResponse.getFailures());
                        return CompletableFuture.supplyAsync(() -> decryptBatchDocuments(encryptedDocuments, dekList))
                                        .thenCombine(CompletableFuture.supplyAsync(() -> getBatchFailures(failureList)),
