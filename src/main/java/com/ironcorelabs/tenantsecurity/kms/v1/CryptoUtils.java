...
class CryptoUtils {
  ...
  public static CompletableFuture<Void> decryptStreamInternal(byte[] documentKey,
      InputStream encryptedStream, OutputStream decryptedStream) {
  ...
  public static Cipher getNewAesCipher(byte[] documentKey, byte[] iv, boolean encryptMode)
    ...
    cipher.init(encryptMode ? Cipher.ENCRYPT_MODE : Cipher.DECRYPT_MODE,
        new SecretKeySpec(documentKey, "AES"), new GCMParameterSpec(GCM_TAG_BIT_LENGTH, iv));
    return cipher;
  }
  ...
  public static CompletableFuture<byte[]> encryptBytes(byte[] document, DocumentMetadata metadata,
    ...
    ByteArrayOutputStream output = new ByteArrayOutputStream(document.length + IV_BYTE_LENGTH
        + GCM_TAG_BYTE_LEN + HEADER_FIXED_SIZE_CONTENT_LENGTH + 512);
    return encryptStreamInternal(documentKey, metadata, new ByteArrayInputStream(document), output,
        secureRandom).thenApply(unused -> {
   ...
   */
  public static CompletableFuture<byte[]> decryptDocument(byte[] encryptedDocumentBytes,
      byte[] documentKey) {
   ...
   *
   * Current version is as follows: VERSION_NUMBER (1 bytes, fixed at 3), IRONCORE_MAGIC (4 bytes,
   * IRON in ASCII), HEADER_LENGTH (2 bytes Uint16), PROTOBUF_HEADER_DATA (variable bytes)
   *
   */
  static CompletableFuture<byte[]> generateHeader(byte[] documentKey, DocumentMetadata metadata,
    ...
    return createHeaderProto(documentKey, metadata, secureRandom)
        .thenCompose(proto -> CompletableFutures.tryCatchNonFatal(() -> {
          ...
          return ByteBuffer.allocate(DOCUMENT_HEADER_META_LENGTH + saasHeaderBytes.length)
              .put(headerVersion).put(magic).put(headerSize).put(saasHeaderBytes).array();
        }));
  }
  ...
  static CompletableFuture<Boolean> verifyHeaderProto(byte[] documentKey,
      DocumentHeader.v3DocumentHeader header) {
    ...
    if (sigBuffer.remaining() == 0) {
      ...
      return CompletableFutures.tryCatchNonFatal(() -> {
        ...
        return generateSignature(documentKey, iv, header.getSaasShield()).thenApply(
            computedGcmBuffer -> computedGcmBuffer.getGcmTag().equals(ByteBuffer.wrap(gcmTag)));
      });
    }
  }
