...
class CryptoUtils {
  ...
  public static CompletableFuture<Void> encryptStreamInternal(
      byte[] documentKey,
    ...
    return generateHeader(documentKey, metadata, secureRandom)
        .thenCompose(
            headerBytes -> {
  ...
  public static CompletableFuture<Void> decryptStreamInternal(
      byte[] documentKey, InputStream encryptedStream, OutputStream decryptedStream) {
    return getHeaderFromStream(encryptedStream)
        .thenCompose(
            header -> {
              return verifyHeaderProto(documentKey, header)
                  .thenCompose(
                      verification -> {
                        return CompletableFutures.tryCatchNonFatal(
                            () -> {
                              if (!verification) {
                                throw new CryptoException(
                                    "The signature computed did not match. Likely that the"
                                        + " documentKey is incorrect.");
                              }
                              byte[] iv = readNBytes(encryptedStream, IV_BYTE_LENGTH);
                              if (iv.length != IV_BYTE_LENGTH) {
                                throw new CryptoException(
                                    "IV not found on the front of the encrypted document.");
                              }
                              Cipher cipher = getNewAesCipher(documentKey, iv, false);
                              byte[] currentChunk = new byte[0];
                              while ((currentChunk = readNBytes(encryptedStream, STREAM_CHUNKING))
                                      .length
                                  > 0) {
  ...
  public static Cipher getNewAesCipher(byte[] documentKey, byte[] iv, boolean encryptMode)
      throws Exception {
  ...
  public static CompletableFuture<byte[]> encryptBytes(
      byte[] document, DocumentMetadata metadata, byte[] documentKey, SecureRandom secureRandom) {
    ...
    ByteArrayOutputStream output =
        new ByteArrayOutputStream(
            document.length
                ...
                + 512);
    return encryptStreamInternal(
            documentKey, metadata, new ByteArrayInputStream(document), output, secureRandom)
        .thenApply(
            unused -> {
   ...
   *
   * @param encryptedDocument The encrypted document including the ICL document header.
   * @param documentKey The AES key to decrypt the document.
   * @return A future with the document decrypted.
   */
  public static CompletableFuture<byte[]> decryptDocument(
      byte[] encryptedDocumentBytes, byte[] documentKey) {
    ByteArrayInputStream encryptedStream = new ByteArrayInputStream(encryptedDocumentBytes);
    ByteArrayOutputStream decryptedStream =
        new ByteArrayOutputStream(encryptedDocumentBytes.length);
   ...
   *
   * <p>Current version is as follows: VERSION_NUMBER (1 bytes, fixed at 3), IRONCORE_MAGIC (4
   * bytes, IRON in ASCII), HEADER_LENGTH (2 bytes Uint16), PROTOBUF_HEADER_DATA (variable bytes)
   */
  static CompletableFuture<byte[]> generateHeader(
      byte[] documentKey, DocumentMetadata metadata, SecureRandom secureRandom) {
    ...
    return createHeaderProto(documentKey, metadata, secureRandom)
        .thenCompose(
            proto ->
                CompletableFutures.tryCatchNonFatal(
                    () -> {
                      ...
                      if (saasHeaderBytes.length > MAX_HEADER_SIZE) {
                        throw new CryptoException(
                            "The header is too large. It is "
                                + saasHeaderBytes.length
                                + " bytes long.");
                      }
                      ...
                      return ByteBuffer.allocate(
                              DOCUMENT_HEADER_META_LENGTH + saasHeaderBytes.length)
                          .put(headerVersion)
                          .put(magic)
                          .put(headerSize)
                          .put(saasHeaderBytes)
  ...
  /**
   * Get the header bytes off of the stream. This will mutate the stream and read up until the
   * cipher text with the IV on the front of it.
   *
   * @param inputStream The input stream to read the header from.
   * @return The document header.
   */
  static CompletableFuture<DocumentHeader.v3DocumentHeader> getHeaderFromStream(
  ...
  static CompletableFuture<DocumentHeader.v3DocumentHeader> createHeaderProto(
      byte[] documentKey, DocumentMetadata metadata, SecureRandom secureRandom) {

    DocumentHeader.SaaSShieldHeader saasHeader =
        DocumentHeader.SaaSShieldHeader.newBuilder().setTenantId(metadata.getTenantId()).build();
    byte[] iv = new byte[IV_BYTE_LENGTH];
    secureRandom.nextBytes(iv);
    return generateSignature(documentKey, iv, saasHeader)
        .thenApply(
            sig -> {
              return DocumentHeader.v3DocumentHeader
                  .newBuilder()
                  .setSaasShield(saasHeader)
                  .setSig(com.google.protobuf.ByteString.copyFrom(sig.getSig()))
                  .build();
            });
  }

  static CompletableFuture<V3HeaderSignature> generateSignature(
      byte[] documentKey, byte[] iv, DocumentHeader.SaaSShieldHeader header) {
    return CompletableFutures.tryCatchNonFatal(
        () -> {
          ByteArrayOutputStream saasHeaderOutput = new ByteArrayOutputStream();
          ...
          ByteBuffer.wrap(encryptResult, encryptResult.length - GCM_TAG_BYTE_LEN, GCM_TAG_BYTE_LEN)
              .get(tag);
          return new V3HeaderSignature(iv, tag);
        });
  }

  static CompletableFuture<Boolean> verifyHeaderProto(
      byte[] documentKey, DocumentHeader.v3DocumentHeader header) {
          ...
          .thenCompose(
              thisIsNull -> {
                ...
                return generateSignature(documentKey, iv, header.getSaasShield())
                    .thenApply(
                        computedGcmBuffer ->
                            computedGcmBuffer.getGcmTag().equals(ByteBuffer.wrap(gcmTag)));
              });
    }
  }
  ...
  public static boolean isCiphertext(byte[] bytes) {
    ...
    return bytes.length >= DOCUMENT_HEADER_META_LENGTH
        && bytes[0] == CURRENT_DOCUMENT_HEADER_VERSION
        && containsIroncoreMagic(bytes)
