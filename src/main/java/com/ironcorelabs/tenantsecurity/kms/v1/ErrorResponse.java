...
public class ErrorResponse {
  ...
  TenantSecurityException toTenantSecurityException(int httpStatusCode) {
    ...
    if (errorCode >= 0 && TenantSecurityErrorCodes.valueOf(errorCode) != null) {
      if (errorCode == 0) {
        return new TspServiceException(TenantSecurityErrorCodes.UNABLE_TO_MAKE_REQUEST,
            httpStatusCode, this.getMessage());
        ...
        return new KmsException(TenantSecurityErrorCodes.valueOf(errorCode), httpStatusCode,
            this.getMessage());
      } else if (errorCode >= 300 && errorCode <= 399) {
        return new SecurityEventException(TenantSecurityErrorCodes.valueOf(errorCode),
            httpStatusCode, this.getMessage());
      } else {
        return new TspServiceException(TenantSecurityErrorCodes.UNKNOWN_ERROR, errorCode,
            this.getMessage());
      }
