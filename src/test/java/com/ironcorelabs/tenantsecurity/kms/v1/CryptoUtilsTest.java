...
public class CryptoUtilsTest {
  ...
  public void generateHeaderShouldProduceKnownResult() throws Exception {
    ...
    ByteBuffer result =
        ByteBuffer.wrap(CryptoUtils.generateHeader(documentKey, metadata, localSecureRandom).get());
    byte[] expectedLength = {(byte) 0, (byte) 42};
    byte[] expectedHeaderProtoBytes = {
      10, 28, 49, 113, -17, 60, -119, -97, -121, 94, 89, 92, 34, 19, -54, -49, -110, -121, -57,
      -116, -15, -106, 69, -116, -42, -112, 84, 73, -128, -57, 26, 10, 10, 8, 116, 101, 110, 97,
      110, 116, 73, 100
    };
  ...
  public void verifyHeaderProtoShouldRoundtripWithCreateHeaderProto() throws Exception {
    ...
    DocumentMetadata metadata =
        new DocumentMetadata(
            ...
            "requestingUserOrServiceId",
            "dataLabel");
    assert (CryptoUtils.verifyHeaderProto(
            documentKey, CryptoUtils.createHeaderProto(documentKey, metadata, secureRandom).get())
        .get());
  }
  ...
  public void streamingEncryptDecryptTest() throws Exception {
    ...
    CryptoUtils.encryptStreamInternal(
            documentKey, metadata, inputStream, encryptOutputStream, secureRandom)
        .get();
  ...
  public void decryptLargeDocumentFromPHPTests() throws Exception {
    ...
    String resultString =
        new String(result); // These bytes were utf-8 bytes so we just load them into a string for
    // our assertion.
  ...
  public void verifyV3DocWithZeroSize() throws Exception {
    byte[] edoc =
        new byte[] {
          3, 73, 82, 79, 78, 0, 0, 52, 97, 69, -17, -65, 32, 85, -70, 101, 109, -67, 31, -28, -38,
          -19, -78, 42, 125, 124, -47, 80, 31, 10, 127, -109, -20, 90, 7, 88, 104, 103, -64, -56,
          38, 95, 96, -97, -92, -54
        };
    ByteArrayInputStream edoc_stream = new ByteArrayInputStream(edoc);
    boolean isHeaderValid =
        CryptoUtils.getHeaderFromStream(edoc_stream)
            .thenCompose(header -> CryptoUtils.verifyHeaderProto(new byte[0], header))
            .get();
  ...
  @Test(
      expectedExceptions = java.util.concurrent.ExecutionException.class,
  ...
  public void streamingEncryptWithNormalDecryptFailureWithBadTag() throws Exception {
    ...
    messedUpEncyptedBytes[length - 2] =
        0; // making the last 2 bytes 0 means the tag should be messed up so the
    // decrypt should fail.
  ...
  @Test(
      ...
      expectedExceptionsMessageRegExp = ".*length.*")
