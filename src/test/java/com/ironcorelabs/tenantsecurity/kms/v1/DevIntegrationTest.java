...
public class DevIntegrationTest {
  ...
  public void missingRandomGen() throws Exception {
    new TenantSecurityClient(
            "http://localhost",
            "apiKey",
            TenantSecurityClient.DEFAULT_REQUEST_THREADPOOL_SIZE,
            TenantSecurityClient.DEFAULT_AES_THREADPOOL_SIZE,
            null)
        .close();
  }
  ...
  private CompletableFuture<TenantSecurityClient> getClient() {
    return TenantSecurityClient.create(
        TestSettings.TSP_ADDRESS + TestSettings.TSP_PORT, this.INTEGRATION_API_KEY);
  }
  ...
  private DocumentMetadata getRoundtripMetadata(String tenantID) {
    ...
    return new DocumentMetadata(
        tenantID, "requestingUserOrServiceID", "dataLabel", arbData, "requestID");
  }
  ...
  public void isCiphertextEncryptedDocTest() throws Exception {
    ...
    CompletableFuture<EncryptedDocument> roundtrip =
        getClient()
            .thenCompose(
                client -> {
                  try {
        ...
        getClient()
            .thenCompose(
                client -> {
                  try {
                    return client
                        .encrypt(originalFields, metadata)
                        .thenCompose(
                            firstEncryptResult -> {
                              PlaintextDocument updatedDoc =
                                  new PlaintextDocument(
                                      updatedFields, firstEncryptResult.getEdek());
                              return client
                                  .encrypt(updatedDoc, metadata)
                                  .thenCompose(
                                      updatedEncryptedResults -> {
                                        // Attempt to decrypt the updated field with the key from
                                        // the first
                  ...
                  try {
                    return client
                        ...
                        .thenCompose(
                            encryptedResults -> {
                              return client
                                  .rekeyDocument(encryptedResults, metadata, this.AZURE_TENANT_ID)
                                  .thenCompose(
                                      rekeyResults -> {
                                        assertEquals(
                                            rekeyResults.getEncryptedFields(),
                                            encryptedResults.getEncryptedFields());
                                        DocumentMetadata newMetadata =
                                            getRoundtripMetadata(this.AZURE_TENANT_ID);
        ...
        getClient()
            .thenCompose(
                client -> {
                  try {
                    return client
                        .encryptBatch(batchRows, metadata)
                        .thenCompose(
                            batchResult -> {
                    ...
                    return client
                        ...
                        .thenCompose(
                            initialEncryptResults -> {
                              ...
                              return client
                                  .encryptExistingBatch(updateBatch, metadata)
                                  .thenCompose(
                                      batchUpdateResult -> {
                                        ...
                                        fullEncryptedUser10.putAll(
                                            initialEncryptResults
                                                .getDocuments()
                                                .get("user10")
                                                .getEncryptedFields());
                                        fullEncryptedUser10.putAll(
                                            updateSuccesses.get("user10").getEncryptedFields());

                                        Map<String, byte[]> fullEncryptedUser30 = new HashMap<>();
                                        fullEncryptedUser30.putAll(
                                            initialEncryptResults
                                                .getDocuments()
                                                .get("user30")
                                                .getEncryptedFields());
                                        fullEncryptedUser30.putAll(
                                            updateSuccesses.get("user30").getEncryptedFields());

                                        Map<String, EncryptedDocument> mergedEncryptedList =
                                            new HashMap<>();
                                        mergedEncryptedList.put(
                                            "user10",
                                        ...
                                        mergedEncryptedList.put(
                                            "user20",
                                            initialEncryptResults.getDocuments().get("user20"));
                                        mergedEncryptedList.put(
                                            "user30",
                                            new EncryptedDocument(
                                                fullEncryptedUser30,
  ...
  public void largeBatchTest() throws Exception {
    ...
    List<Map<String, byte[]>> rows =
        Arrays.stream(new int[batchSize]).mapToObj(_nope -> document).collect(Collectors.toList());
        ...
        getClient()
            .thenCompose(
                client -> {
                  try {
                    return client
                        .encryptBatch(batch, metadata)
                        .thenCompose(
                            batchEncryptedResults -> {
                              assertEquals(0, batchEncryptedResults.getFailures().size());
                              assertEquals(batchSize, batchEncryptedResults.getDocuments().size());
                              return client.decryptBatch(
                                  batchEncryptedResults.getDocuments(), metadata);
                            });
                  } catch (Exception e) {
                    throw new CompletionException(e);
                  }
  ...
  public void leasedDataTest() throws Exception {
    ...
    String leasedDocumentEdek =
        "";

    String leasedDocumentBytes =
        "A0lST04ANgocVzWAgU8k159ETeoE+b9F0wkZFjn6psKQhQ8dWhoWChRJTlRFR1JBVElPTi1URVNULUdDUNRZlwKbH7ZHtnQ2hdiGdNBJhqIC0p+hqMnzRO7AymqrCo8ohE8bIscVfTFk";
  ...
  public void logSecurityEvent() throws Exception {
    EventMetadata metadata =
        new EventMetadata(this.GCP_TENANT_ID, "integrationTest", "sample", "app-request-id");
