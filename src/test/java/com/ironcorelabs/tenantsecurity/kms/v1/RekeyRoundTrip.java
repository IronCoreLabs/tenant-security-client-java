...
public class RekeyRoundTrip {
  ...
  // These match up to the Demo TSP whose config we ship with the repo in
  ...
  public void roundtripTest() throws Exception {
    ...
    CompletableFuture<PlaintextDocument> roundtrip =
        TenantSecurityClient.create(tsp_address + tsp_port, api_key)
            .thenCompose(
                client -> {
                  try {
                    return client
                        ...
                        .thenCompose(
                            encryptedDocument -> {
                              return client
                                  .rekeyDocument(encryptedDocument, metadata, new_tenant_id)
                                  .thenCompose(
                                      encryptedDocumentRekeyed -> {
                                        assertEquals(
                                            encryptedDocumentRekeyed.getEncryptedFields(),
                                        ...
                                        return client.decrypt(
                                            encryptedDocumentRekeyed, newMetadata);
                                      });
                            });
                  } catch (Exception e) {
