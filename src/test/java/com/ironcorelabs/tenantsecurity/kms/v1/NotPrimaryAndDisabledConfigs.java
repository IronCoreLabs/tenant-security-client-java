...
public class NotPrimaryAndDisabledConfigs {
   ...
   */
  private byte[] existingEncryptedDataForEnabledConfig =
      new byte[] {
        3, 73, 82, 79, 78, 0, 56, 10, 28, -48, -74, -114, -45, -58, -105, 111, 15, 20, -110, 86, -6,
        -86, 109, 36, -48, -115, -13, -127, -13, -86, 77, 31, 118, -92, -54, 16, -94, 26, 24, 10,
        22, 73, 78, 84, 69, 71, 82, 65, 84, 73, 79, 78, 45, 84, 69, 83, 84, 45, 67, 79, 77, 66, 79,
        26, 52, -85, -26, 127, 70, -1, -80, -97, 101, -68, 45, -66, -31, 125, -66, 32, -95, -11,
        113, -86, 72, 115, -24, -52, -114, -28, -112, 123, 83, 16, 30, 107, -60, -16, -125, 72, 45,
        101, -30, 100, 39, 126, -40
      };
  ...
  /**
   * Data encrypted from a previous configuration. The configuration this data was encrypted to has
   * been disabled. We should no longer be able to ask the Tenant Security Proxy to decrypt this
   * data. This data was encrypted via an Azure configuration.
   */
  private byte[] existingEncryptedDataForDisabledConfig =
      new byte[] {
        3, 73, 82, 79, 78, 0, 56, 10, 28, -83, 13, 21, -61, -47, -109, 53, -78, -26, 76, 22, -111,
        -22, -21, -79, 90, -118, 107, -122, 69, -111, 126, 42, 75, -66, 66, -10, -58, 26, 24, 10,
        22, 73, 78, 84, 69, 71, 82, 65, 84, 73, 79, 78, 45, 84, 69, 83, 84, 45, 67, 79, 77, 66, 79,
        -66, 77, -58, 69, -61, 57, 110, -68, 68, -6, -51, -95, 118, 93, -68, -74, 71, -72, 93, 82,
        58, 122, 76, 104, 33, -52, -41, -95, -35, -43, 56, -10, 123, 122, -108, 5, 71, 71, -34, -64,
        50, -37, 4, 27
      };
  ...
  private CompletableFuture<TenantSecurityClient> getClient() {
    return TenantSecurityClient.create(
        TestSettings.TSP_ADDRESS + TestSettings.TSP_PORT,
  ...
  private DocumentMetadata getRoundtripMetadata() {
    ...
    return new DocumentMetadata(
        NotPrimaryAndDisabledConfigs.MULTIPLE_TENANT_ID, "integrationTest", "sample", arbData);
  }
  ...
  public void decryptActiveConfigData() throws Exception {
    ...
    CompletableFuture<PlaintextDocument> decrypted =
        getClient()
            .thenCompose(
                client ->
                    client.decrypt(
                        this.getExistingEncryptedDataForEnabledConfig(),
    ...
    CompletableFuture<PlaintextDocument> decrypted =
        getClient()
            .thenCompose(
                client ->
                    client.decrypt(
                        this.getExistingEncryptedDataForDisabledConfig(),
                        this.getRoundtripMetadata()));
